...
var serviceNames = { apple: 'Apple Music', spotify: 'Spotify', deezer: 'Deezer', elite: 'Deezer', library: 'Library' };
var musicTypes = ['album', 'song', 'station', 'load'];
...
function getAccountId(player, service) {
  ...
  if (service != 'library') {
    return request({ url: player.baseUrl + '/status/accounts', json: false }).then(function (res) {
...
function musicSearch(player, values) {
  ...
  return getAccountId(player, service).then(function () {
    ...
    if (serviceDef.empty(type, resList)) {
      ...
      } else if (type == 'album' && service != 'library') {
        UaM = serviceDef.urimeta(type, resList);
