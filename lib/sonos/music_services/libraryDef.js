...
function loadLibrary(player) {
  ...
  var getChunk = function getChunk(chunk) {
    ...
    return player.browse('A:TRACKS', chunk.startIndex + chunk.numberReturned, 0).then(getChunk);
  };

  return Promise.resolve(result).then(getChunk);
}
